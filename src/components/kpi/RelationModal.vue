  ...
  <div class="modal-overlay" @click="closeModal">
    <div class="modal-container" @click.stop>
      <div class="modal-header">
        <h2>Agregar Relación entre Campos</h2>
      ...
      <div class="modal-content">
        <form @submit.prevent="saveRelation" class="relation-form">
          <div class="form-group">
            <label for="field1_id">Primer Campo *</label>
            <select
              id="field1_id"
              v-model="relationData.field1_id"
              :class="{ 'error': errors.field1_id }"
            >
            ...
            <span v-if="errors.field1_id" class="error-message">{{ errors.field1_id }}</span>
          </div>
          ...
          <div class="form-group">
            <label for="field2_id">Segundo Campo *</label>
            <select
              id="field2_id"
              v-model="relationData.field2_id"
              :class="{ 'error': errors.field2_id }"
            >
            ...
            <span v-if="errors.field2_id" class="error-message">{{ errors.field2_id }}</span>
          </div>
          ...
          <div v-if="relationData.field1_id && relationData.operation && relationData.field2_id" class="formula-preview">
            <h4>Vista Previa de la Fórmula:</h4>
            <div class="formula-display">
              <span class="field">{{ getFieldName(relationData.field1_id) }}</span>
              <span class="operator">{{ relationData.operation }}</span>
              <span class="field">{{ getFieldName(relationData.field2_id) }}</span>
            </div>
          </div>
        </form>
      </div>
...
const isFormValid = computed(() => {
  return relationData.field1_id && 
         relationData.operation && 
         relationData.field2_id && 
         relationData.order > 0 &&
         relationData.field1_id !== relationData.field2_id
})
...
const getNextOrder = () => {
  if (props.relations.length === 0) return 1
  const maxOrder = Math.max(...props.relations.map(r => r.order || 0))
  return maxOrder + 1
}
...
const saveRelation = () => {
  if (isFormValid.value) {
    ...
    if (relationData.field1_id === relationData.field2_id) {
      errors.field2_id = 'Los campos deben ser diferentes'
      return
    }

    // Validar orden único
    const existingOrder = props.relations.find(r => r.order === relationData.order)
    if (existingOrder) {
...
// Inicializar orden automáticamente
relationData.order = getNextOrder()
</script>
